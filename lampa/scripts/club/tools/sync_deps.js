...
const SUMMARY_FILE = path.join(ROOT_DIR, 'sync_deps.summary.json');
...
const DEFAULT_MAX_TOTAL_FILES = 5_000;
...
const MAX_TOTAL_FILES = clampInt(
  process.env.SYNC_DEPS_MAX_TOTAL_FILES,
  1,
  200_000,
  DEFAULT_MAX_TOTAL_FILES,
);
const MAX_DEPTH = clampInt(
  process.env.SYNC_DEPS_MAX_DEPTH,
  0,
  50,
  DEFAULT_MAX_DEPTH,
);
const MAX_TEXT_SCAN_BYTES = clampInt(
  process.env.SYNC_DEPS_MAX_TEXT_SCAN_BYTES,
  64 * 1024,
  200 * 1024 * 1024,
...
function isTrackingUrl(urlObj) {
  const p = String(urlObj.pathname || '').toLowerCase();
  const q = String(urlObj.search || '').toLowerCase();
  if (/\b(utm_[a-z0-9_]+|gclid|fbclid)=/.test(q)) return true;
  if (/(^|\/)(ads|click|track|analytics|pixel)(\/|$)/.test(p)) return true;
  if (/(?:\?|&)(ads|click|track|analytics|pixel)=/.test(q)) return true;
  return false;
}

function isDynamicPaginationUrl(urlObj) {
  try {
    const sp = urlObj.searchParams;
    return sp.has('page') || sp.has('offset') || sp.has('cursor');
  } catch {
    return false;
  }
}
...
const DEEP_EXTENSIONLESS_ALLOW_PREFIXES = new Set([
  'club',
  'sisi',
  'rch',
  'cors',
...
function isProbablyDynamicQueryUrl(urlObj) {
  if (!urlObj || !urlObj.search || urlObj.search === '') return false;
  const ext = path.posix.extname(urlObj.pathname || '');
  if (ext) return false; // query on static asset is usually cache-busting
  try {
    const keys = [...urlObj.searchParams.keys()].map((k) => k.toLowerCase());
    if (!keys.length) return false;
    return keys.some((k) => !STATIC_QUERY_KEYS.has(k));
  } catch {
    return true;
  }
}
...
function decideSaveWebPath(urlObj, contentType) {
  ...
  if (hasExt) return pathname;

  // Rule: preserve the URL path as-is. Only add index.* for directory-like paths.
  if (!endsWithSlash) return pathname;

  const mime = String(contentType || '').split(';')[0].trim().toLowerCase();
  const inferredExt = extFromContentType(mime) || '.bin';
  return pathname + 'index' + inferredExt;
}
...
async function httpGetFollow(urlObj, logStream, redirectsLeft) {
  ...
  if (
    ...
    if (next.protocol !== 'http:' && next.protocol !== 'https:') {
      logLine(logStream, `redirect blocked (scheme): ${urlObj.toString()} -> ${next.toString()}`);
      res.resume();
      throw makeBlockedError(
        'BLOCKED_REDIRECT_SCHEME',
        `Redirect blocked to non-http(s): ${next.toString()}`,
        { blocked_url: next.toString() },
      );
    }
    if (!isAllowedHostname(next.hostname)) {
      logLine(logStream, `redirect blocked (external): ${urlObj.toString()} -> ${next.toString()}`);
      res.resume();
      throw makeBlockedError(
        'EXTERNAL_BLOCKED_REDIRECT',
        `Redirect blocked to external host: ${next.toString()}`,
        { blocked_url: next.toString() },
      );
    }
...
async function downloadToTemp(urlObj, logStream) {
  if (!isAllowedHostname(urlObj.hostname)) {
    throw makeBlockedError(
      'EXTERNAL_BLOCKED',
      `External host blocked: ${urlObj.toString()}`,
      { blocked_url: urlObj.toString() },
    );
  }
...
function rewriteJsText(jsText, ctx) {
  ...
  const rewriteUrlString = (rawUrl, baseUrl) => {
    ...
    if (urlObj.hostname && !isAllowedHostname(urlObj.hostname)) return null;
    const abs = urlObj.toString();
    const pathname = urlObj.pathname || '/';

    // Prefer exact mapping when available
    const mapped = urlToLocal.get(abs);
    if (mapped && mapped.local_path) return mapped.local_path;

    // Strip origin for any url with path
    if (pathname && pathname !== '/') return pathname + (urlObj.search || '');

    // origin-only: rewrite to empty string to avoid protocol/domain in code
  ...
  const replaceInChunk = (chunk, opts) => {
    ...
    updated = updated.replace(
      /((?:https?:\/\/|wss?:\/\/|\/\/)[^\s"'`<>)[\]]+|(?:^|[^a-zA-Z0-9_])((?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}(?::\d{1,5})?\/[^\s"'`<>)[\]]+))/g,
      (m, g1, g2) => {
        const raw = g1 || g2;
...
function rewriteGenericText(text, ctx) {
  ...
  out = out.replace(
    /((?:https?:\/\/|wss?:\/\/|\/\/)[^\s"'`<>)[\]]+|(?:^|[^a-zA-Z0-9_])((?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}(?::\d{1,5})?\/[^\s"'`<>)[\]]+)|\/[^\s"'`<>)[\]]+\?[^\s"'`<>)[\]]+)/g,
    (m, g1, g2) => {
      const raw = g1 || g2 || m;
...
function postProcessClubJs(jsText) {
  ...
  out = out.replace(
    /\bvar\s+hostkey\s*=\s*(['"])[^'"]*\1\s*\.\s*replace\(\s*(['"])http:\/\/\2\s*,\s*(['"])\3\s*\)\s*\.\s*replace\(\s*(['"])https:\/\/\4\s*,\s*(['"])\5\s*\)\s*;/g,
    () => {
      changed += 1;
...
async function main() {
  ...
  // Migrate legacy map keys to current schema
  for (const [remote, meta] of Object.entries(mapObj)) {
    if (!meta || typeof meta !== 'object') continue;
    if (!meta.original_url && typeof remote === 'string') meta.original_url = remote;
    if (!meta.local_path && typeof meta.local_web_path === 'string')
      meta.local_path = meta.local_web_path;
    if (!meta.local_fs_path && typeof meta.local_fs === 'string') meta.local_fs_path = meta.local_fs;
    if (!meta.sha256 && typeof meta.content_hash === 'string') meta.sha256 = meta.content_hash;
    if (!meta.bytes && Number.isFinite(meta.size)) meta.bytes = meta.size;
    if (!meta.content_type && typeof meta.detected_type === 'string')
      meta.content_type = meta.detected_type;
    if (!meta.referrer && Array.isArray(meta.sources) && meta.sources.length) {
      const last = meta.sources[meta.sources.length - 1];
      if (last && last.from) meta.referrer = last.from;
    }
  ...
  for (const [remote, meta] of Object.entries(mapObj)) {
    ...
    if (typeof meta.local_path !== 'string') continue;
    urlToLocal.set(remote, meta);
    if (meta.local_path && meta.local_path.startsWith('/')) {
      reverseLocalToRemote.set(meta.local_path, remote);
    }
  }
  ...
  logLine(
    logStream,
    `start: concurrency=${CONCURRENCY} retries=${RETRIES} timeout_ms=${TIMEOUT_MS} max_redirects=${MAX_REDIRECTS} max_bytes=${MAX_BYTES} max_urls=${MAX_URLS} max_total_files=${MAX_TOTAL_FILES} max_depth=${MAX_DEPTH} max_text_scan_bytes=${MAX_TEXT_SCAN_BYTES} include_html_links=${INCLUDE_HTML_LINKS} origin=${DEFAULT_ORIGIN}`,
  );

  const entryText = readFileUtf8Safe(ENTRY_FILE);

  // Determine default origin (force allowlist)
  let defaultOrigin = DEFAULT_ORIGIN;
  const envOrigin = String(process.env.SYNC_DEPS_ORIGIN || '').trim();
  if (envOrigin) {
    try {
      const u = new URL(envOrigin);
      if (
        (u.protocol === 'http:' || u.protocol === 'https:') &&
        isAllowedHostname(u.hostname)
      ) {
  ...
  const knownOrigins = new Set([defaultOrigin]);
  for (const remote of urlToLocal.keys()) {
    try {
      const u = new URL(remote);
      if (isAllowedHostname(u.hostname)) knownOrigins.add(u.origin);
    } catch {}
  }
  for (const raw of extractUrlLikeFromTextChunk(entryText)) {
    const u = normalizeCandidateToUrl(raw, null, null);
    if (u && isAllowedHostname(u.hostname)) knownOrigins.add(u.origin);
  }
  ...
  const recordSkip = (abs, status, sourceInfo, depth, extra) => {
    const e = ensureMapEntry(abs);
    e.status = status;
    if (Number.isFinite(depth)) e.depth = depth;
    if (sourceInfo && sourceInfo.from) e.referrer = sourceInfo.from;
    addSource(e, sourceInfo);
    if (extra && typeof extra === 'object') Object.assign(e, extra);
  };

  const addUrlTask = (urlObj, sourceInfo, depth) => {
    if (!urlObj) return;
    const abs = urlObj.toString();
    ...
    if (seen.has(abs)) {
      const existing = urlToLocal.get(abs) || mapObj[abs];
    ...
    if (depth > MAX_DEPTH) {
      skipped.depth += 1;
      recordSkip(abs, 'skipped_depth', sourceInfo, depth, {
        error: `max_depth=${MAX_DEPTH}`,
      });
      return;
    }
    ...
    if (urlObj.protocol === 'ws:' || urlObj.protocol === 'wss:') {
      if (isAllowedHostname(urlObj.hostname)) {
        skipped.ws += 1;
        recordSkip(abs, 'needs_manual_ws', sourceInfo, depth, {});
      } else {
        skipped.external += 1;
        recordSkip(abs, 'external_blocked', sourceInfo, depth, {});
      }
      return;
    }
    ...
    if (urlObj.protocol !== 'http:' && urlObj.protocol !== 'https:') {
      ...
      recordSkip(abs, 'skipped_unsupported_scheme', sourceInfo, depth, {});
      return;
    }
    ...
    if (String(urlObj.pathname || '').toLowerCase().startsWith('/proxyimg')) {
      skipped.dynamic += 1;
      recordSkip(abs, 'skipped_dynamic', sourceInfo, depth, { error: 'proxyimg' });
      return;
    }

    if (isTrackingUrl(urlObj)) {
      skipped.tracking += 1;
      recordSkip(abs, 'skipped_tracking', sourceInfo, depth, {});
      return;
    }

    if (isDynamicPaginationUrl(urlObj)) {
      skipped.dynamic += 1;
      recordSkip(abs, 'skipped_dynamic', sourceInfo, depth, {});
      return;
    }

    if (isProbablyDynamicQueryUrl(urlObj)) {
      skipped.dynamic += 1;
      recordSkip(abs, 'skipped_dynamic', sourceInfo, depth, {
        error: 'non_static_query',
      });
      return;
    }

    // Avoid crawling deep extensionless navigation/content paths by default
    if (!path.posix.extname(urlObj.pathname || '')) {
      const parts = String(urlObj.pathname || '')
        .split('/')
        .filter(Boolean);
      if (parts.length >= 2) {
        const first = String(parts[0] || '').toLowerCase();
        if (!DEEP_EXTENSIONLESS_ALLOW_PREFIXES.has(first)) {
          skipped.dynamic += 1;
          recordSkip(abs, 'skipped_dynamic', sourceInfo, depth, {
            error: 'deep_extensionless_path',
          });
          return;
        }
      }
    ...
    if (!shouldDownloadUrl(urlObj)) {
      recordSkip(abs, 'skipped_no_resource_path', sourceInfo, depth, {});
      return;
    }

    if (queuedDownloadables >= MAX_TOTAL_FILES) {
      skipped.limit += 1;
      recordSkip(abs, 'skipped_limit', sourceInfo, depth, {
        error: `max_total_files=${MAX_TOTAL_FILES}`,
      });
      return;
    }

    queuedDownloadables += 1;
    queue.push({ url: abs, source: sourceInfo, depth });
  };

  const addDynamic = (pattern, sourceInfo, depth) => {
    if (!pattern) return;
    ...
    mapObj[key] = {
      ...
      referrer: (sourceInfo && sourceInfo.from) || '',
      local_path: '',
      local_fs_path: '',
      final_url: '',
      error: '',
  ...
  for (const [localWeb, remote] of reverseLocalToRemote.entries()) {
    if (entryText.includes(localWeb)) {
      try {
        addUrlTask(
          new URL(remote),
          { from: 'club.js', kind: 'map_ref', raw: localWeb },
          0,
        );
      } catch {}
    }
  ...
  for (const raw of allSeedRaws) {
    ...
    if (raw && raw.startsWith('/') && reverseLocalToRemote.has(raw)) {
      ...
      try {
        addUrlTask(new URL(remote), { from: 'club.js', kind: 'local_ref', raw }, 0);
      } catch {}
      continue;
    }
    ...
    if (!u) continue;
    addUrlTask(u, { from: 'club.js', kind: 'seed', raw }, 0);
  }
  ...
  const processTask = async (task) => {
    ...
    const existing = urlToLocal.get(absUrl) || mapObj[absUrl];
    if (
      existing &&
      existing.local_fs_path &&
      fs.existsSync(path.join(ROOT_DIR, existing.local_fs_path))
    ) {
      addSource(existing, task.source);
      const existingWebPath = existing.local_path || '';
      const existingType = existing.content_type || '';
      // Still scan for links if it's text and not scanned
      if (
        existingWebPath &&
        (isTextWebPath(existingWebPath) || isProbablyTextByContentType(existingType))
      ) {
        const absFs = path.join(ROOT_DIR, existing.local_fs_path);
        if (!scannedText.has(absFs)) {
          scannedText.add(absFs);
          await scanDownloadedText(
            absFs,
            existingWebPath,
            absUrl,
            existingType,
            Number.isFinite(existing.depth) ? existing.depth : task.depth,
          );
        }
      }
      return;
    ...
    for (let attempt = 0; attempt <= RETRIES; attempt += 1) {
      try {
        ...
        const httpCodeMatch = msg.match(/^HTTP\s+(\d{3})\b/);
        ...
        const permanentBlocked =
          e &&
          (e.code === 'EXTERNAL_BLOCKED' ||
            e.code === 'EXTERNAL_BLOCKED_REDIRECT' ||
            e.code === 'BLOCKED_REDIRECT_SCHEME');
        const shouldRetry = attempt < RETRIES && !permanentHttp && !permanentBlocked;
        ...
        const entry = ensureMapEntry(absUrl);
        entry.depth = task.depth;
        if (task.source && task.source.from) entry.referrer = task.source.from;
        addSource(entry, task.source);
        entry.error = msg;
        if (e && e.code === 'EXTERNAL_BLOCKED_REDIRECT') {
          skipped.external += 1;
          entry.status = 'external_blocked_redirect';
          if (e.blocked_url) entry.blocked_url = e.blocked_url;
        } else if (e && e.code === 'BLOCKED_REDIRECT_SCHEME') {
          skipped.unsupported += 1;
          entry.status = 'blocked_redirect_scheme';
          if (e.blocked_url) entry.blocked_url = e.blocked_url;
        } else {
          entry.status = 'failed_download';
          failed.push({ url: absUrl, error: msg, depth: task.depth, referrer: entry.referrer });
        }
        return;
      }
    }
    ...
    // Query: always include a stable suffix to avoid overwrites.
    if (urlObj.search && urlObj.search !== '') {
      const qh = sha1Hex(urlObj.search).slice(0, 8);
      saveWebPath = addSuffixBeforeExt(saveWebPath, `__q_${qh}`);
    }

    // Resolve file/dir conflicts (e.g. "/x" as file and "/x/y" as dir)
    const convertExistingFileToDirIndex = async (fileWebPath) => {
      let fileAbs;
      try {
        fileAbs = safeFsPathFromWebPath(fileWebPath);
      } catch {
        return;
      }
      ...
      let ct = '';
      const remote = reverseLocalToRemote.get(fileWebPath);
      const meta = remote ? urlToLocal.get(remote) || mapObj[remote] : null;
      if (meta) ct = meta.content_type || meta.detected_type || '';

      const idxExt =
        extFromContentType(String(ct).split(';')[0].trim().toLowerCase()) || '.bin';
      const idxWebPath = sanitizeWebPath(fileWebPath + '/index' + idxExt);
      const idxAbs = safeFsPathFromWebPath(idxWebPath);

      ensureDirSync(TMP_DIR);
      const tmpAbs = path.join(
        TMP_DIR,
        'mv_' + crypto.randomBytes(6).toString('hex'),
      );

      await fs.promises.rename(fileAbs, tmpAbs);
      fs.mkdirSync(fileAbs);
      await moveFileSafe(tmpAbs, idxAbs);

      logLine(logStream, `path-fix: file->dir ${fileWebPath} -> ${idxWebPath}`);

      if (remote && meta) {
        meta.local_path = idxWebPath;
        meta.local_fs_path = path.relative(ROOT_DIR, idxAbs);
        reverseLocalToRemote.delete(fileWebPath);
        reverseLocalToRemote.set(idxWebPath, remote);
      }
    };

    const ensureParentsAreDirs = async (webPath) => {
      const safeWeb = sanitizeWebPath(webPath);
      const parts = safeWeb.replace(/^\/+/, '').split('/').filter(Boolean);
      let cur = '';
      for (let i = 0; i < parts.length - 1; i += 1) {
        cur += '/' + parts[i];
        let absCur;
        try {
          absCur = safeFsPathFromWebPath(cur);
        } catch {
          continue;
        }
        if (!fs.existsSync(absCur)) continue;
        try {
          const st = fs.statSync(absCur);
          if (st.isFile()) await convertExistingFileToDirIndex(cur);
        } catch {}
      }
    };

    await ensureParentsAreDirs(saveWebPath);

    // If destination already exists as a directory, store as /path/index.*
    try {
      const maybeAbs = safeFsPathFromWebPath(saveWebPath);
      if (fs.existsSync(maybeAbs) && fs.statSync(maybeAbs).isDirectory()) {
        const idxExt =
          extFromContentType(
            String(detectedType).split(';')[0].trim().toLowerCase(),
          ) || '.bin';
        const idxWebPath = sanitizeWebPath(saveWebPath + '/index' + idxExt);
        logLine(logStream, `path-fix: dest is dir ${saveWebPath} -> ${idxWebPath}`);
        saveWebPath = idxWebPath;
        await ensureParentsAreDirs(saveWebPath);
      }
    } catch {}
    ...
    const entry = ensureMapEntry(absUrl);
    entry.status = status;
    entry.local_path = saveWebPath;
    entry.local_fs_path = destRel;
    entry.sha256 = dl.contentHash;
    entry.bytes = dl.size;
    entry.content_type = detectedType || dl.headerContentType || '';
    entry.content_type_header = dl.headerContentType || '';
    ...
    if (isTextWebPath(saveWebPath) || isProbablyTextByContentType(detectedType)) {
      ...
      if (!scannedText.has(absFs)) {
        scannedText.add(absFs);
        await scanDownloadedText(absFs, saveWebPath, absUrl, detectedType, task.depth);
      }
    }
  };

  const scanDownloadedText = async (absFsPath, webPath, remoteUrl, detectedType, depth) => {
    try {
      const st = fs.statSync(absFsPath);
      if (Number.isFinite(st.size) && st.size > MAX_TEXT_SCAN_BYTES) {
        logLine(
          logStream,
          `scan skipped (too large): ${webPath} bytes=${st.size} limit=${MAX_TEXT_SCAN_BYTES}`,
        );
        return;
      }
    } catch {
    ...
    else if (mime === 'application/javascript' || mime === 'text/javascript') type = 'js';
    ...
    if (type === 'js') {
      ...
      for (const d of js.dynamicTemplates) {
        addDynamic(d.pattern, { from: webPath, kind: 'js_template', raw: d.pattern }, depth + 1);
      }
      ...
      for (const raw of all) {
        enqueueRawCandidate(raw, remoteUrl, webPath, 'js', depth + 1);
      }
      return;
    }
    ...
    for (const it of extracted) {
      ...
      if (type === 'html' && it.kind === 'a') {
        ...
        if (!u) continue;
        addUrlTask(u, { from: webPath, kind: 'html_a', raw: it.value }, depth + 1);
        logLine(logStream, `html<a>: ${webPath} -> ${u.toString()}`);
        continue;
      }
      enqueueRawCandidate(it.value, remoteUrl, webPath, it.kind || type, depth + 1);
    }
  };

  const enqueueRawCandidate = (raw, baseRemoteUrl, fromWebPath, kind, depth) => {
    if (!raw) return;
    ...
    if (trimmed.startsWith('/') && reverseLocalToRemote.has(trimmed)) {
      ...
      try {
        addUrlTask(new URL(remote), { from: fromWebPath, kind: 'local_ref', raw: trimmed }, depth);
      } catch {}
      return;
    }
    ...
    if (!u) return;
    addUrlTask(u, { from: fromWebPath, kind, raw: trimmed }, depth);
  };
  ...
  const rewriteOneFile = (absFsPath, fileType, baseUrl) => {
    ...
    if (text !== old) {
      ...
      rewrittenFiles.add(path.relative(ROOT_DIR, absFsPath));
  ...
  const rewriteAllTextFiles = () => {
    rewriteOneFile(ENTRY_FILE, 'js', null);

    // Rewrite all text files we downloaded (from map)
    for (const meta of Object.values(mapObj)) {
      if (!meta || typeof meta !== 'object') continue;
      if (!meta.local_fs_path || !meta.local_path) continue;
      const webPath = meta.local_path;
      const ct = meta.content_type || meta.detected_type || '';
      if (!isTextWebPath(webPath) && !isProbablyTextByContentType(ct)) continue;
      ...
      const isJs =
        ...
        mime === 'application/javascript' ||
        mime === 'text/javascript';
      rewriteOneFile(absFs, isJs ? 'js' : 'text', meta.original_url || meta.final_url);
    }
  };

  const extractLocalRefsFromText = (text) => {
    const out = new Set();
    const s = String(text || '');
    for (const m of s.matchAll(
      ...
      if (idx > 0) {
        const prev = s[idx - 1];
        // ignore HTML closing tags and "word/word" fragments inside identifiers
  ...
  const verifyProject = (opts) => {
    const enqueueMissing = Boolean(opts && opts.enqueue_missing);
    const processed = [];
    processed.push({
      abs: ENTRY_FILE,
      rel: path.relative(ROOT_DIR, ENTRY_FILE) || 'club.js',
      depth: 0,
      isJs: true,
    });
    ...
    for (const meta of Object.values(mapObj)) {
      if (!meta || typeof meta !== 'object') continue;
      if (!meta.local_fs_path) continue;
      const abs = path.join(ROOT_DIR, meta.local_fs_path);
      ...
      processed.push({
        abs,
        rel: meta.local_fs_path,
        depth: Number.isFinite(meta.depth) ? meta.depth : 1,
        isJs,
      });
    }

    const ab2024Left = [];
    const missingAgg = new Map(); // ref -> {count, referrers:Set}
    ...
    for (const f of processed) {
      ...
      if (f.isJs) {
        walkJs(text, {
          onString: (raw) => {
            for (const r of extractLocalRefsFromText(raw)) refs.add(r);
          },
          onTemplateChunk: (raw) => {
            for (const r of extractLocalRefsFromText(raw)) refs.add(r);
          },
        ...
        for (const r of extractLocalRefsFromText(text)) refs.add(r);
      }

      for (const ref of refs) {
        const cleaned = String(ref).split('#')[0];
        if (!cleaned.startsWith('/') || cleaned.startsWith('//') || cleaned === '/') continue;

        const noQuery = cleaned.split('?')[0];
        const candidates = [noQuery];
        if (noQuery.endsWith('/')) {
          candidates.push(noQuery + 'index.html');
          candidates.push(noQuery + 'index.json');
          candidates.push(noQuery + 'index.txt');
          candidates.push(noQuery + 'index.js');
          candidates.push(noQuery + 'index.css');
          candidates.push(noQuery + 'index.bin');
        } else {
          candidates.push(noQuery + '/index.html');
          candidates.push(noQuery + '/index.json');
          candidates.push(noQuery + '/index.txt');
        ...
        const key = cleaned;
        const cur = missingAgg.get(key) || { ref: key, count: 0, referrers: new Set() };
        cur.count += 1;
        cur.referrers.add(f.rel);
        missingAgg.set(key, cur);

        if (enqueueMissing) {
          let remote = '';
          if (reverseLocalToRemote.has(key)) {
            remote = reverseLocalToRemote.get(key);
          } else {
            try {
              remote = new URL(key, defaultOrigin).toString();
            } catch {
              remote = '';
            }
          }
          if (remote) {
            try {
              addUrlTask(
                new URL(remote),
                { from: f.rel, kind: 'missing_local', raw: key },
                (Number.isFinite(f.depth) ? f.depth : 0) + 1,
              );
            } catch {}
          }
        }
    ...
    const missingLocal = [...missingAgg.values()]
      .sort((a, b) => b.count - a.count)
      .map((m) => ({
        ref: m.ref,
        count: m.count,
        referrers: [...m.referrers].slice(0, 10),
      }));

    return {
      processed_files: processed.length,
      ab2024_left_files: ab2024Left,
      missing_local: missingLocal,
      enqueued_missing: queue.length - beforeQueue,
    };
  };

  rewriteAllTextFiles();

  const verifyPass1 = verifyProject({ enqueue_missing: true });
  if (verifyPass1.enqueued_missing > 0) {
    logLine(logStream, `missing_local: queued=${verifyPass1.enqueued_missing} pass=1`);
    await schedule();
    rewriteAllTextFiles();
  }

  const verifyPass2 = verifyProject({ enqueue_missing: false });
  ...
  const bucketForContentType = (ct) => {
    ...
    if (mime.startsWith('font/') || mime.includes('woff') || mime.includes('font'))
      return 'font';
  ...
  const downloadedByType = {};
  for (const e of Object.values(mapObj)) {
    if (!e || typeof e !== 'object') continue;
    if (e.status !== 'downloaded' && e.status !== 'skipped_same') continue;
    const bucket = bucketForContentType(e.content_type || e.detected_type || '');
    downloadedByType[bucket] = (downloadedByType[bucket] || 0) + 1;
  }

  const summary = {
    origin: defaultOrigin,
    discovered_urls: seen.size,
    max_total_files: MAX_TOTAL_FILES,
    max_depth: MAX_DEPTH,
    max_text_scan_bytes: MAX_TEXT_SCAN_BYTES,
    downloaded_count: downloadedCount,
    downloaded_by_type: downloadedByType,
    rewritten_files: rewrittenFiles.size,
    rewritten_occurrences: totalReplaced,
    dynamic_unresolved: totalDynamicUnresolved,
    skipped_external_count: skipped.external,
    skipped_tracking_count: skipped.tracking,
    skipped_dynamic_count: skipped.dynamic,
    skipped_depth_count: skipped.depth,
    skipped_limit_count: skipped.limit,
    needs_manual_ws_count: skipped.ws,
    failed_download_count: failed.length,
    ab2024_left_files: verifyPass2.ab2024_left_files,
    missing_local: verifyPass2.missing_local.slice(0, 200),
    verify_missing_club_js: verify.missing.slice(0, 200),
    failed_downloads: failed.slice(0, 200),
    top_domains: topDomains(domainCounts, 10),
    ms,
  };
  ...
  logLine(
    logStream,
    `done: discovered=${seen.size} downloaded=${downloadedCount} rewritten_files=${rewrittenFiles.size} rewritten_total=${totalReplaced} dynamic_unresolved=${totalDynamicUnresolved} skipped_external=${skipped.external} skipped_tracking=${skipped.tracking} skipped_dynamic=${skipped.dynamic} failed_download=${failed.length} missing_local=${verifyPass2.missing_local.length} ab2024_left_files=${verifyPass2.ab2024_left_files.length} verify_missing=${verify.missing.length} ms=${ms}`,
  );
  ...
  console.log(
    `- downloaded by type: ${
      Object.keys(downloadedByType).length
        ? Object.entries(downloadedByType)
            .sort((a, b) => b[1] - a[1])
            .map(([k, v]) => `${k}=${v}`)
            .join(', ')
        : '(none)'
  ...
  console.log(`- rewritten files: ${rewrittenFiles.size}`);
  console.log(`- rewritten occurrences: ${totalReplaced}`);
  console.log(`- dynamic_unresolved (templates): ${totalDynamicUnresolved}`);
  console.log(
    `- skipped: external=${skipped.external} tracking=${skipped.tracking} dynamic=${skipped.dynamic} depth=${skipped.depth} limit=${skipped.limit} ws=${skipped.ws}`,
  );
  ...
  console.log(`- ab2024 leftovers (files): ${verifyPass2.ab2024_left_files.length}`);
  if (verifyPass2.ab2024_left_files.length) {
    for (const f of verifyPass2.ab2024_left_files.slice(0, 50)) {
      console.log(`  - ${f.file}: ${f.count}`);
    }
    if (verifyPass2.ab2024_left_files.length > 50)
      console.log(
        `  ... and ${verifyPass2.ab2024_left_files.length - 50} more`,
      );
  }

  console.log(`- missing_local: ${verifyPass2.missing_local.length}`);
  if (verifyPass2.missing_local.length) {
    for (const m of verifyPass2.missing_local.slice(0, 30)) {
      console.log(`  - ${m.ref} (x${m.count}) <- ${m.referrers.join(', ')}`);
    }
    if (verifyPass2.missing_local.length > 30)
      console.log(`  ... and ${verifyPass2.missing_local.length - 30} more`);
  }

  console.log(`- failed_download: ${failed.length}`);
  if (failed.length) {
    for (const f of failed.slice(0, 30)) {
      const from = f.referrer ? ` <- ${f.referrer}` : '';
      console.log(`  - ${f.url}: ${f.error}${from}`);
    }
  ...
  console.log(`- summary: ${path.relative(ROOT_DIR, SUMMARY_FILE)}`);
