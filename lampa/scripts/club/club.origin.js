(function () {
  ...
  function modal(text) {
    ...
    var content =
      '<div class="about">\n<div>' +
      (text || 'Добавьте идентификатор устройства в init.conf') +
  ...
  function play(element) {
    ...
    if (
      ...
      !element.history_uid &&
      element.bookmark &&
      Lampa.Storage.field('sisi_history')
    ) {
      network.silent(
        Api.account(Defined.localhost + '/history/add'),
        function (e) {},
        function () {},
    ...
    if (element.json) {
                ...
                if (a.json) {
                  Api.qualitys(a.video, function (data) {
                    ...
                    if (data.qualitys_proxy)
                      a.url_reserve = Api.account(
                        qualityDefault(data.qualitys_proxy),
                        true,
                      );
                    call();
                  });
      ...
      var video = {
        title: element.name,
        url: Api.account(
          qualityDefault(element.qualitys) || element.video,
          true,
        ),
        url_reserve: Api.account(
          qualityDefault(element.qualitys_proxy) || element.video_reserve || '',
          true,
        ),
  ...
  function fixCards(json) {
    json.forEach(function (m) {
      ...
      m.poster = m.picture;
      m.img = m.picture;
      m.name = Lampa.Utils.capitalizeFirstLetter(m.name).replace(
        /\&(.*?);/g,
        '',
      );
    });
    ...
    Lampa.Select.show({
      ...
      onSelect: function onSelect(m) {
        if (m.model) {
          Lampa.Activity.push({
            url:
              Defined.localhost.replace('/sisi', '') +
              '/' +
              card_data.model.uri,
  ...
  function ApiPWA() {
    ...
    this.menu = function (success, error) {
      ...
      DotNet.invokeMethodAsync('JinEnergy', 'sisi', '')
        .then(function (data) {
          if (data) {
            menu$1 = data;
            success(menu$1);
    ...
    this.view = function (params, success, error) {
      var u = this.account(
        Lampa.Utils.addUrlComponent(params.url, 'pg=' + (params.page || 1)),
      );
      DotNet.invokeMethodAsync('JinEnergy', u.path, u.query)
        .then(function (json) {
          if (json.list) {
    ...
    this.account = function (u, join) {
      if (join) {
        if (
          Defined.use_api == 'lampac' &&
          u.indexOf(Defined.localhost.replace('/sisi', '')) == -1
        )
          return u;
      }
    ...
    this.playlist = function (add_url_query, oncomplite, error) {
      var load = function load() {
        ...
        menu$1.forEach(function (m, i) {
          ...
          var url_query =
            add_url_query.indexOf('?') !== -1 ||
            add_url_query.indexOf('&') !== -1
              ? add_url_query.substring(1)
              : add_url_query;
  ...
  function ApiHttp() {
    ...
    this.menu = function (success, error) {
      ...
      network.silent(
        this.account(Defined.localhost) +
          '&rchtype=' +
    ...
    this.view = function (params, success, error, waiting_rch) {
      var u = Lampa.Utils.addUrlComponent(
        params.url,
        'pg=' + (params.page || 1),
      );
      network.silent(
    ...
    this.bookmark = function (element, add, call) {
      var u =
        Defined.localhost +
        '/bookmark/' +
        (add ? 'add' : 'remove?id=' + element.bookmark.uid);
      network.silent(
        this.account(u),
        function (e) {
          call(true);
    ...
    this.account = function (u) {
      ...
      if (profile_id != '')
        u = Lampa.Utils.addUrlComponent(u, 'profile_id=' + profile_id);
    ...
    this.qualitys = function (video_url, oncomplite, error, waiting_rch) {
      network.silent(
        this.account(video_url + '&json=true'),
        function (json) {
          if (json.rch) {
            if (waiting_rch) {
              error();
  ...
  function Sisi(object) {
    ...
    comp.empty = function (er) {
      ...
      var empty = new Lampa.Empty({
        descr:
          typeof er == 'string' ? er : Lampa.Lang.translate('empty_text_two'),
      });
      ...
      Api.view(
        ...
        function (data) {
          ...
          if (!data.results.length && object.url.indexOf('/bookmarks')) {
            Lampa.Noty.show(
              'Удерживайте ОК на видео для добавления в закладки.',
              {
                time: 10000,
              },
            );
  ...
  function startPlugin() {
    ...
    function addFilter() {
      ...
      Lampa.Listener.follow('activity', function (e) {
        ...
        if (
          e.type == 'start' &&
          e.component == 'sisi_view_' + Defined.use_api
        ) {
          button.show();
          activi = e.object;
        }
    ...
    function add() {
      var button = $(
        '<li class="menu__item selector" data-action="sisi">\n            <div class="menu__ico">\n                <svg width="200" height="243" viewBox="0 0 200 243" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M187.714 130.727C206.862 90.1515 158.991 64.2019 100.983 64.2019C42.9759 64.2019 -4.33044 91.5669 10.875 130.727C26.0805 169.888 63.2501 235.469 100.983 234.997C138.716 234.526 168.566 171.303 187.714 130.727Z" stroke="currentColor" stroke-width="15"/><path d="M102.11 62.3146C109.995 39.6677 127.46 28.816 169.692 24.0979C172.514 56.1811 135.338 64.2018 102.11 62.3146Z" stroke="currentColor" stroke-width="15"/><path d="M90.8467 62.7863C90.2285 34.5178 66.0667 25.0419 31.7127 33.063C28.8904 65.1461 68.8826 62.7863 90.8467 62.7863Z" stroke="currentColor" stroke-width="15"/><path d="M100.421 58.5402C115.627 39.6677 127.447 13.7181 85.2149 9C82.3926 41.0832 83.5258 35.4214 100.421 58.5402Z" stroke="currentColor" stroke-width="15"/><rect x="39.0341" y="98.644" width="19.1481" height="30.1959" rx="9.57407" fill="currentColor"/><rect x="90.8467" y="92.0388" width="19.1481" height="30.1959" rx="9.57407" fill="currentColor"/><rect x="140.407" y="98.644" width="19.1481" height="30.1959" rx="9.57407" fill="currentColor"/><rect x="116.753" y="139.22" width="19.1481" height="30.1959" rx="9.57407" fill="currentColor"/><rect x="64.9404" y="139.22" width="19.1481" height="30.1959" rx="9.57407" fill="currentColor"/><rect x="93.0994" y="176.021" width="19.1481" height="30.1959" rx="9.57407" fill="currentColor"/></svg>\n            </div>\n            <div class="menu__text">' +
          Lampa.Lang.translate('lampac_sisiname') +
          '</div>\n        </li>',
      );
          ...
          function () {
            Api.menu(
              function (data) {
                ...
                if (
                  true &&
                  (Defined.use_api !== 'pwa' || Lampa.Platform.is('android'))
                ) {
                  items.push({
                    title: 'Все',
                  });
