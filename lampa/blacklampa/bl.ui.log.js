(function () {
	'use strict';
		...
		// Logging mode (affects ONLY auto-popup/live output; recording is always enabled).
		// Storage key contract (do not change without migration):
		// - localStorage['bl_log_mode_v1'] = 'silent' | 'popup'
		var MODE_LS_KEY = 'bl_log_mode_v1';
		var MODE_SILENT = 'silent';
		var MODE_POPUP = 'popup';

		// Legacy numeric alias for existing code paths:
		// - 0: silent (no auto popup)
		// - 1: popup  (auto popup allowed)
		var LOG_MODE = 0;
		var LOG_MODE_STR = MODE_SILENT;

		// In-memory ring buffer (session-only).
		// Must be cheap: no shift(), bounded memory, no DOM work unless viewer/popup is enabled.
		var RING_MAX = 1000;
	...
	var TAG_STYLE = {
		...
		function readModeFromStorage() {
			...
			if (m === null || m === undefined || m === '') {
				// Persist default to avoid "flicker" and to make the choice explicit.
		...
		function applyMode(mode) {
			...
			// Keep legacy fast alias for other modules (must NOT be used as "logging enabled" gate anymore).
			try {
				if (BL.cfg && typeof BL.cfg === 'object') BL.cfg.LOG_MODE = LOG_MODE;
			} catch (_) { }

			// If popup already exists, refresh dynamic bits (mode/title) and cleanup on silent.
			safe(function () {
				if (!popupEl) return;
				try {
				...
				// When switching to silent, never keep auto-popup visible/timers.
		...
		// Apply persisted mode as early as possible (before any heavy log/UI hooks).
	...
	function hidePopup() {
		...
		function renderRingToPopup() {
			try {
				...
				for (var i = 0; i < snap.length; i++) {
					var it = snap[i];
					if (!it) continue;
					frag.appendChild(makeRow(it.line, it.tag));
				}
		...
		try { if (typeof uiCfg.popupProgressHeightPx === 'number') POPUP_PROGRESS_HEIGHT_PX = uiCfg.popupProgressHeightPx; } catch (_) { }
			...
			// Load + apply persisted mode early (no query params).
			applyMode(readModeFromStorage());
		...
		function setViewerMode(on) {
			...
			if (!el || !popupScrollEl) return;
				...
				try { if (popupCloseEl) popupCloseEl.style.display = 'none'; } catch (_) { }
					...
					// Shrink/cleanup DOM when leaving viewer to keep UI light.
					safe(function () {
