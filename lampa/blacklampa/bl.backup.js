(function () {
  ...
  var HISTORY_KEY = 'bl_backup_history_v1';
  var HISTORY_MAX = 80;
  ...
  function getCfg() {
    ...
    // Ensure config exists on disk (for first-run / schema migration).
  ...
  API.history.add = function (item) {
    ...
    // De-dup by provider+id (keep latest on top)
    try {
  ...
  // ----------------------------------------------------------------------------
  // Helpers (not part of public API contract, but useful for UI)
  // ----------------------------------------------------------------------------
