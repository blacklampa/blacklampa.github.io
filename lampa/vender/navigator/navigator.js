...
var SpatialNavigator = function () {
  this._focus = null;
  this._previous = null;
  this._lastCanMove = null;
  this._distanceTargetRect = null;
  ...
  this._distanceFunction = {
    ...
    nearPlumbLineIsBetter: function (rect) {
      var targetRect = self._distanceTargetRect;
      var d;
...
SpatialNavigator.prototype = {
  ...
  _getAllRects: function snGetAllRects(excludedElem) {
    ...
    if (!collection || !collection.length) return rects;
  ...
  _partition: function snDemarcate(rects, targetRect) {
    ...
    for (var i = 0, l = rects.length; i < l; i++) {
      ...
      if (groupId === 0 || groupId === 2 || groupId === 6 || groupId === 8) {
  ...
  _getDistanceFunction: function snGetDistanceFunction(targetRect) {
    this._distanceTargetRect = targetRect;
  ...
  _prioritize: function snPrioritize(priorities, target, direction) {
    ...
    for (var i = 0, l = priorities.length; i < l; i++) {
      if (priorities[i].group && priorities[i].group.length) {
    ...
    if (this.rememberSource &&
      ...
      for (var k = 0, kl = destPriority.group.length; k < kl; k++) {
        if (destPriority.group[k].element == source) {
          return destPriority.group[k];
        }
      }
    }

    var distances = destPriority.distance;
    destPriority.group.sort(function (a, b) {
      for (var i = 0, l = distances.length; i < l; i++) {
        var diff = distances[i](a) - distances[i](b);
        if (diff) return diff;
      }
      return 0;
    });
  ...
  setCollection: function snSetCollection(collection) {
    ...
    if (Array.isArray && Array.isArray(collection)) {
      this._collection = collection.slice(0);
      return;
    }

    if (collection.length !== undefined && typeof collection !== 'string') {
      this._collection = Array.prototype.slice.call(collection);
      return;
    }
  ...
  multiAdd: function snMultiAdd(elements) {
    ...
    var list = elements;
    if (!(Array.isArray && Array.isArray(elements)) && !(elements.length !== undefined && typeof elements !== 'string')) {
      list = [elements];
    }

    var success = true;
    for (var i = 0, l = list.length; i < l; i++) {
      if (!this.add(list[i])) success = false;
    }
    return success;
  },
  ...
  multiRemove: function snMultiRemove(elements) {
    ...
    var success = true;
    for (var i = 0, l = list.length; i < l; i++) {
      if (!this.remove(list[i])) success = false;
    }
    return success;
  },
  ...
  move: function snMove(direction) {
    ...
    if (!this._focus) {
      ...
      var dir = direction && direction.toLowerCase ? direction.toLowerCase() : direction;
      var cached = this._lastCanMove;
      var elem = cached && cached.source === this._focus && cached.direction === dir && cached.ts && Date.now() - cached.ts < 100 ? cached.dest : this.navigate(this._focus, direction);
      this._lastCanMove = null;
      if (!elem) {
        return false;
      }
  ...
  canmove: function (direction) {
    if (this._focus) {
      ...
      this._lastCanMove = {
        source: this._focus,
        direction: direction && direction.toLowerCase ? direction.toLowerCase() : direction,
        dest: elem,
        ts: Date.now()
      };
  ...
  navigate: function snNavigate(target, direction) {
    ...
    switch (direction) {
      case 'left':
        priorities = [
          {
            group: internalGroups[0].concat(internalGroups[3], internalGroups[6]),
            distance: [
      ...
      case 'right':
        priorities = [
          {
            group: internalGroups[2].concat(internalGroups[5], internalGroups[8]),
            distance: [
              distanceFunction.nearPlumbLineIsBetter,
              distanceFunction.topIsBetter
            ]
      ...
      case 'up':
        priorities = [
          {
            group: internalGroups[0].concat(internalGroups[1], internalGroups[2]),
            distance: [
              distanceFunction.nearHorizonIsBetter,
              distanceFunction.leftIsBetter
            ]
      ...
      case 'down':
        priorities = [
          {
            group: internalGroups[6].concat(internalGroups[7], internalGroups[8]),
            distance: [
              distanceFunction.nearHorizonIsBetter,
              distanceFunction.leftIsBetter
            ]
...
var Navigator = new SpatialNavigator();
